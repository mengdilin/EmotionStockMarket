...
def test(name):
    ...
    count=otterapi.pre_set(name,times[6],times[7])[1]
    time=otterapi.pre_set(name,times[6],times[7])[0]
...
def market_setup():
    ...
    for name in names:
        ...
        stock={"stock":name,"last count":count,"data":[{"time":time,"price":40}]}
        market.insert(stock)
